    ...
    Function Set-Transcription
{
    ...
    $NetPath = "\\storageserver.ad.schots.com.au\OFS\ICT\Elliot\PowerShell\Transcript\" + $env:ComputerName
    $Path = $env:USERPROFILE + "\Documents\WindowsPowerShell\Transcript"
    # Turn on PowerShell transcripting. 
    Start-Transcript -Path "$NetPath\$Filename" | Out-Null
}
    ...
    Write-Host " `tComputerName:`t`t" -nonewline -ForegroundColor Green;Write-Host $($env:COMPUTERNAME)"`t`t`t" -nonewline -ForegroundColor Cyan;Write-Host "UserName:`t" -nonewline -ForegroundColor Green;Write-Host $env:UserDomain\$env:UserName"`t`t" -nonewline -ForegroundColor Cyan;Write-Host " " -ForegroundColor Green
